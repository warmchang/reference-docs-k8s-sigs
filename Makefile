...
EXTAPIDST=$(WEBROOT)/content/en/docs/reference/external-api/
KUBELETAPIDST=$(WEBROOT)/content/en/docs/reference/kubelet-api/

# APIs defined by the project but served outside kube-apiserver; these are
# published under external-api/ instead of config-api/.
EXTAPIPAGES=metrics.v1 metrics.v1beta1 custom-metrics.v1beta2 external-metrics.v1beta1

# APIs the kubelet serves over HTTP; these are published under kubelet-api/.
KUBELETAPIPAGES=stats.v1alpha1

# Generated by genref but not published on the website.
CONFIGSKIP=apiserver-resourcequota.v1 cloud-provider-config.v1alpha1 \
	cloud-provider-service.v1alpha1 kube-scheduler-extender.v1

configapi:
	make -C genref all

copyconfigapi: require-webroot configapi
	for p in $(EXTAPIPAGES); do cp $(CONFIGSRC)/$$p.md $(EXTAPIDST); done
	for p in $(KUBELETAPIPAGES); do cp $(CONFIGSRC)/$$p.md $(KUBELETAPIDST); done
	for f in $(CONFIGSRC)/*.md; do \
		b=`basename $$f .md`; \
		case " $(EXTAPIPAGES) $(KUBELETAPIPAGES) $(CONFIGSKIP) " in *" $$b "*) continue;; esac; \
		cp $$f $(CONFIGDST); \
	done
